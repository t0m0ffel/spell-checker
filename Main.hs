...
{-g''::Tree (Char,[(Char,Int)])->Tree String
g'' t = fmap (\x-> (show x)) t

g'''::Tree (Char,Int,Bool)->Tree String
g''' t = fmap (\x-> (show x)) t

a t = putStrLn $ drawTree  (g''' t)


wr t = writeFile "b.txt" (drawTree $ g' t)

-- ok
                        --putStrLn $ show  $ length $ flatten $ z dicf
z dicf =  insertAll $ words dicf



g t = putStrLn $ drawTree  (g' t)

g'::Tree (Char,Int,Bool)->Tree String
g' t = fmap (\(x,y,z)->if z then "$"++(show (x,y))++"$" else (show (x,y))) t


ausgabe word dicf = mapM_ print  $ take 20 $ expe $ readOff $ insertWord (init word) (insertAll $ words dicf )
-- $ map (\(x,_)->x) 


mehr::Tree (Char,Bool)->[String]->[[String]]
mehr  t w = go w
 where 
        go::[String]->[[String]]
        go [] = []
        go (x:xs) = if checkWord x t
                                then ["!!!!!!"]:go xs
                                else map (\(x,y)-> show (x,y)) (take 30 $ expe $ readOff $ insertWord (init x) t):go xs

                readOff::Tree (Char,Int,Bool)->[[(Char,Int,Bool)]]
                readOff (Node (x) []) =  [[(x)]]
                readOff (Node (x) xs) =  [(x):summary | t <- xs , summary <- readOff t]





                expe::[[(Char,Int,Bool)]]->[(String,Int)]
                expe xs =  (sortBy (comparing  snd) $ nub $ go xs)
                 where
                                go::[[(Char,Int,Bool)]]->[(String,Int)]
                                go []  = []
                                go (x:xs)  = (exp' x)++go xs

                exp'::[(Char,Int,Bool)]->[(String,Int)]
                exp' xs = go xs "" []
                 where
                                go::[(Char,Int,Bool)]->String->[(String,Int)]->[(String,Int)]
                                go [] _ ss = ss
                                go ((x,y,z):xs) s ss
                                                                                |x==' ' = go xs s ss
                                                                                |True = if z
                                                                                                then go xs (s++[x]) (((s++[x]),y):ss)
                                                                                                else go xs (s++[x]) ss
